...
5. In terminal: `open Carthage` opens the carthage file in a text editor (don't use textedit)
...
NOTE: Sometimes you have to point Xcode correctly: `sudo xcode-select -s <path to Xcode 9 beta>/Xcode-beta.app/Contents/Developer`
NOTE: App targets needs to have the framework refs in `embedded libraries` (framework targets do not)
...
4. in terminal: build .framework and .dynlib via the command: `carthage build --no-skip-current` (this step is only needed if users wants to skip buillidng from src)  
...
https://stackoverflow.com/a/49787323/5389500
Using private repo https://samwize.com/2018/06/27/creating-a-private-framework-with-carthage/
